...
for i_part = 1:length(exp.participants) % separate trials by errors
    ...
    %errors on each trial by mean+/-SD errors
...
for i_elect = 1:(length(exp.singletrialselecs)+1) %add one for Avg O1 & O2
    ...
    % Plot Difference: Large-Small Errors
    figure; colormap('jet')
...
%     line([-517 -517],[min(freqs) max(freqs)],'color','m','LineStyle','--') %vertical line for mask onset
    line([-567 -567],[min(freqs) max(freqs)],'color','b','LineStyle','--') %vertical line for target onset
    ylim([1 30]); xlim([-1600 600]); xticks(-1600:200:600)
...
%finds the times you want from the timess variable
% timewin = [-600 -400]; i_elect = 2; %Pz %alpha (p=0.0.0262)
% timewin = [-550 -450];
% timewin = [-900 -700]; i_elect = 6; %O1 & O2 %alpha (p=0.0225)
timewin = [-600 -400]; i_elect = 6; %O1 & O2 %alpha (p=0.0466)
% timewin = [0 50]; i_elect = 2; %Pz %theta (p=0.0280)
timelim = find(times>=timewin(1) & times<=timewin(2));
