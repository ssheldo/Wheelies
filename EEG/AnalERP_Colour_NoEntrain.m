...
for ii = 1:length(plot_ths)
    ...
    title([exp.elec_names{i_elect} ': ERPs by Response Error']); 
    xlabel('Time (ms)'); ylabel('Voltage (uV)'); xticks(xmin:300:xmax);
    legend('Large','Small');
    clear ymin ymax
end
...
% Set the range of time to consider
% itlims = [-250 0]; i_elect = 7; %Pz (p=0.0106)
% itlims = [-250 0]; i_elect = 17; %O1 & O2 (p=0.0362)
% itlims = [-200 100]; i_elect = 7; %Pz (p=0.0093)
% itlims = [300 600];
% itlims = [250 550];
% itlims = [500 600];
% itlims = [450 650];
...
% Start trig -> Entrainer Trig = 0.7
% S 4 -> Target = 1.433
% S 1 -> Target = 0.7410
% S 3 -> Target = 0.8260

(EEG.urevent(42).latency-EEG.urevent(44).latency)*(1/EEG.srate)
