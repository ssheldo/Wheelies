...
% /////////////////////////////////////////////////////////////////////////
%%                  ERPs by Color - Medians
% /////////////////////////////////////////////////////////////////////////
for i_part = 1:length(exp.participants)   
    for ii = 1:length(exp.electrode)
        i_elect = exp.electrode(ii); %because FCz is 17
        
        erp_out_c(i_part,ii,:,1) = median(ALLEEG(i_part).data(i_elect,:,find(targ_colorgrp{i_part}==1)),3);
        erp_out_c(i_part,ii,:,2) = median(ALLEEG(i_part).data(i_elect,:,find(targ_colorgrp{i_part}==2)),3);
        erp_out_c(i_part,ii,:,3) = median(ALLEEG(i_part).data(i_elect,:,find(targ_colorgrp{i_part}==3)),3);
    end
end

erp_out_bycolor(:,:,1) = squeeze(median(erp_out_c(:,:,:,1),1));
erp_out_bycolor(:,:,2) = squeeze(median(erp_out_c(:,:,:,2),1));
erp_out_bycolor(:,:,3) = squeeze(median(erp_out_c(:,:,:,3),1));
...
figure
plot(EEG.times,smooth(erp_out_bycolor(i_elect,:,1),0.008),'g','LineWidth',1.5)
hold on
plot(EEG.times,smooth(erp_out_bycolor(i_elect,:,3),0.008),'-.r','LineWidth',1.5)
hold on
plot(EEG.times,smooth(erp_out_bycolor(i_elect,:,2),0.008),'b','LineWidth',1)
hold on
set(gca,'ydir','reverse'); xlim([-500 1000]); ylim([-4 8])
line([-500 1000],[0 0],'color','k') %horizontal line
line([0 0],[-4 8],'color','k') %vertical line
title([exp.elec_names{i_elect} ': ERPs by Color Trials']); xlabel('Time (ms)'); ylabel('Voltage (uV)'); legend('Green','Red','Blue');

 

figure;
boundedline(EEG.times,squeeze(median(erp_out_c(:,i_elect,:,1),1)),...
    squeeze(std(erp_out_c(:,i_elect,:,1),[],1))./sqrt(nparts),'g',...
    EEG.times,squeeze(median(erp_out_c(:,i_elect,:,2),1)),...
    squeeze(std(erp_out_c(:,i_elect,:,2),[],1))./sqrt(nparts),'r',...
    EEG.times,squeeze(median(erp_out_c(:,i_elect,:,3),1)),...
...
% /////////////////////////////////////////////////////////////////////////
%%                  ERPs by Errors - Median
% /////////////////////////////////////////////////////////////////////////

for i_part = 1:length(exp.participants) % ------------------------------------
    % Get error +/-1 IQR of the median
    ...
    for ii = 1:length(exp.electrode)
        ...
        erp_out_x(i_part,ii,:) = squeeze(median(ALLEEG(i_part).data(i_elect,:,[find((resp_errdeg{i_part}<uplim & resp_errdeg{i_part}>lowlim))]),3));
        % Get trials with small errors
        erp_out_n(i_part,ii,:) = squeeze(median(ALLEEG(i_part).data(i_elect,:,[find(resp_errdeg{i_part}>=uplim) find(resp_errdeg{i_part}<=lowlim)]),3));
    end
    clear mean_errdeg sd_errdeg uplim lowlim
end

erp_out_byerr(:,:,1) = squeeze(median(erp_out_x(:,:,:),1));
erp_out_byerr(:,:,2) = squeeze(median(erp_out_n(:,:,:),1));
...
hold on
set(gca,'ydir','reverse'); xlim([-500 1000]); ylim([-4 10])
line([-500 1000],[0 0],'color','k') %horizontal line
line([0 0],[-4 10],'color','k') %vertical line
title([exp.elec_names{i_elect} ': ERPs by Response Error']); 
xlabel('Time (ms)'); ylabel('Voltage (uV)'); xticks(-500:250:1000);
legend('Large','Small');
...
figure;
boundedline(EEG.times,squeeze(median(erp_out_x(:,i_elect,:),1)),...
    squeeze(iqr(erp_out_x(:,i_elect,:),1))./2,'c',...
    EEG.times,squeeze(median(erp_out_n(:,i_elect,:),1)),...
    squeeze(iqr(erp_out_n(:,i_elect,:),1))./2,'m');
hold on
set(gca,'ydir','reverse'); xlim([-500 1000]); ylim([-8 10])
line([-500 1000],[0 0],'color','k') %horizontal line
line([0 0],[-8 10],'color','k') %vertical line
...
% (EEG.urevent(24).latency-EEG.urevent(22).latency)*(1/EEG.srate)
...
line([50 50],[ymin ymax],'color','b') %vertical line for mask onset
line([567 567],[ymin ymax],'color','b') %vertical line for color wheel onset
set(gca,'ydir','reverse'); xlim([xmin xmax]); ylim([ymin ymax])
title([exp.elec_names{i_elect} ': ERPs by Response Error']); 
xlabel('Time (ms)'); ylabel('Voltage (uV)'); xticks(xmin:250:xmax);
legend('Large','Small');
...
for epp = 1:length(ths_elect)
    i_elect = ths_elect(epp);
    figure;
    boundedline(EEG.times,squeeze(mean(erp_out_x(:,i_elect,:),1)),...
        squeeze(std(erp_out_x(:,i_elect,:),[],1))./sqrt(nparts),'c',...
        EEG.times,squeeze(mean(erp_out_n(:,i_elect,:),1)),...
        squeeze(std(erp_out_n(:,i_elect,:),[],1))./sqrt(nparts),'m');
    hold on
    line([-500 1000],[0 0],'color','k') %horizontal line
    line([0 0],[-8 10],'color','k') %vertical line
    line([50 50],[-8 10],'color','b') %vertical line for mask onset
