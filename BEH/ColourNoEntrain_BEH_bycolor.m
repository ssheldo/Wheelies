...
% List of subject IDs
% parts = {'108';'109';'110';'111';'112';'113';'114';'115';'116'};
parts = {'108';'109';'110';'112';'113';'114';'115';'116'};
...
% /////////////////////////////////////////////////////////////////////////
% =========================================================================
for i_part = 3:length(parts) %loop through subjects
% for i_part = 3:5 %loop through subjects

    % Load each subject's data
    load(['M:\Experiments\Colour\Entrain_Colour_Wheel\Trials\Colour_NoEntrain_Data\' parts{i_part} '_Colour_NoEntrain_v4.mat'])
...
%     
%     g_out_g(i_part,1) = model_out_g{i_part,1}.maxPosterior(1);
%     sd_out_g(i_part,1) = model_out_g{i_part,1}.maxPosterior(2);
%     
%     g_out_b(i_part,1) = model_out_b{i_part,1}.maxPosterior(1);
%     sd_out_b(i_part,1) = model_out_b{i_part,1}.maxPosterior(2);
%     
%     g_out_r(i_part,1) = model_out_r{i_part,1}.maxPosterior(1);
%     sd_out_r(i_part,1) = model_out_r{i_part,1}.maxPosterior(2);
...
% 
    ...
    clear -regexp ^target ^lag ^error_ ^color_ %everything that begins with
end


model = AllGuessingModel; %for when the fitting doesn't work
% When fit fails
model_out{i_part,1} = MemFit(error_deg,model); %just plots
g_out(i_part,1) = NaN;
sd_out(i_part,1) = NaN;
...
for i_part = 1:length(parts)
    neg_mean(i_part,1) = nanmean(out_error_deg{i_part}(out_error_deg{i_part}<=0));
    neg_std(i_part,1) = nanstd(out_error_deg{i_part}(out_error_deg{i_part}<=0));
    pos_mean(i_part,1) = nanmean(out_error_deg{i_part}(out_error_deg{i_part}>0));
    pos_std(i_part,1) = nanstd(out_error_deg{i_part}(out_error_deg{i_part}>0));
    bot_std(i_part,1) = nanstd(out_error_deg{i_part});
...
[h,p]=ttest(g_out_g,g_out_b)
[h,p]=ttest(g_out_g,g_out_r)
[h,p]=ttest(g_out_b,g_out_r)


figure; boxplot( tmpmean)
